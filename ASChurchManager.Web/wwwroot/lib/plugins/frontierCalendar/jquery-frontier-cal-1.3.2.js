...
(function ($) {
		...
		*/
        ...
        this.getWeekIndex = function (dayName) {
            if (dayName.toUpperCase() == "DOM" || dayName.toUpperCase() == "DOMINGO") {
                return 0;
            } else if (dayName.toUpperCase() == "SEG" || dayName.toUpperCase() == "SEGUNDA") {
                return 1;
            } else if (dayName.toUpperCase() == "TER" || dayName.toUpperCase() == "TERÇA") {
                return 2;
            } else if (dayName.toUpperCase() == "QUA" || dayName.toUpperCase() == "QUARTA") {
                return 3;
            } else if (dayName.toUpperCase() == "QUI" || dayName.toUpperCase() == "QUINTA") {
                return 4;
            } else if (dayName.toUpperCase() == "SEX" || dayName.toUpperCase() == "SEXTA") {
                return 5;
    ...
    Calendar.showMoreAgendaModal = function (cal, date, agendaItems) {
        ...
        var spanMessage = "Agenda para " + date.toLocaleDateString("pt-BR");
        // create modal dialog
        var modal = $("<div/>");
        modal.attr("id", modalId);
        //modal.attr("title", spanMessage);
        modal.dialog({
            autoOpen: false,
            height: 400,
            width: 600,
            ...
            resizable: false,
            center: true,
            position: { my: "center", at: "center", of: window, collision: "fit", using: { horizontal: "center", vertical: "left", important: "horizontal" } },
            hide: {
                effect: "blind",
                duration: 1000
            },
        ...
        var agi = null;
        var agendaDiv = null;
        modal.append("<span>" + spanMessage + "</span>");
        modal.append("<p/>");
        ...
        for (var i = 0; i < agendaItems.length; i++) {
            ...
            if (agi.isAllDay()) {
                // don't show start time
                agendaDiv.append("(Evento de Dia Todo)<br>");
            } else {
                agendaDiv.append("De " + agendaStartDt.toLocaleDateString("pt-BR") + " às " + DateUtil.getAgendaDisplayTime(agendaStartDt) +
                    " até " + " " + agendaEndDt.toLocaleDateString("pt-BR") + " às " + DateUtil.getAgendaDisplayTime(agendaEndDt) + "<br>");
            }
            // add click event
            ...
            elements = elements.add(agendaDiv);
        }

        $("#modalItemAgenda h4.modal-title").html("");
        $("#modalItemAgenda h4.modal-title").append("<span>" + spanMessage + "</span>");
        $("#modalItemAgenda div#conteudo").html("");
        $("#modalItemAgenda div#conteudo").append(elements);


        $("#modalItemAgenda").modal({ backdrop: "static" });
        //modal.dialog('open');
